...
class AirSimDroneEnv(gym.Env):
    def __init__(self, ip_address="127.0.0.1", step_length=0.25):
        ...
        self.current_step = 0  # Counter to track the current step
    ...
    def _setup_flight(self):
        ...
        #self.drone.moveByVelocityAsync(1, 2, 0, 5).join()
        self.drone.moveByVelocityAsync(-2.5, -2, 0, 0.5).join()
    ...
    def _get_obs(self):
        ...
        #    self.state["velocity"].x_val, self.state["velocity"].y_val, self.state["velocity"].z_val
        #], dtype=np.float32)

        # Combine position/velocity data with sensor data
        #obs = np.append(obs, [distance_obs])
    ...
    #             )
    #         reward = -10 if dist > thresh_dist else math.exp(-beta * dist) - 0.5
    ...
    def _compute_reward(self):
        ...
        #pts = [np.array([0, 0, -5]),np.array([12.5, 12.5, -5]), np.array([-5, 12.5, -5]), np.array([1.5, -10, -5]), np.array([0, 0, -5])]
        pts = [np.array([0, 0, -5]),np.array([-25, -25, -5])]
    ...
    #     for i in range(0, len(pts) - 1):
    #         dist = min(dist, np.linalg.norm(np.cross((quad_pt - pts[i]), (quad_pt - pts[i + 1]))) / np.linalg.norm(pts[i] - pts[i + 1]))
    #     reward_dist = -dist  # Penalize distance to the line
    #     reward_speed = 0.1*(np.linalg.norm([self.state["velocity"].x_val, self.state["velocity"].y_val, self.state["velocity"].z_val,])- 0.5)
    #     reward = reward_dist + reward_speed

    #     # Avoid obstacles using distance sensor data
    #     sensor_data = self.drone.getDistanceSensorData()
    #     distance = sensor_data.distance
    #     safe_distance = 10  # Define a safe threshold
    #     obstacle_penalty = -10 * max(0, safe_distance - distance) if distance < safe_distance else 0
    #     reward += obstacle_penalty

    #     if self.state["collision"]:
    #         reward -= 100
    #     return reward, self.state["collision"]


    def interpret_action(self, action):
        sensor_data = self.drone.getDistanceSensorData()
        distance = sensor_data.distance
        # Debugging: Print sensor data to verify
        #print(f"Distance Sensor Reading: {distance}")

        # If an obstacle is in front, trigger an evasive maneuver
        #if self.drone.getDistanceSensorData().distance<20:  # Moving forward with an obstacle in front
            #print("MUR")
        #    return (0, -self.step_length, 0)  # Example: move left instead of forward
        
        #if self.drone.getBarometerData().altitude < 3:
